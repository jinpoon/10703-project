...
NOTE_OFF = 0
NO_EVENT = -1 #not used in 703 project
...
class RLTuner(object):
  ...
  def __init__(self, output_dir,
    ...
    if initialize_immediately:
      self.initialize_internal_models_graph_session(restore_from_checkpoint=True)
  ...
  def initialize_internal_models_graph_session(self,
    ...
    with self.graph.as_default():
      ...
      self.target_q_network = note_rnn_loader.NoteRNNLoader(
          ...
          hparams=self.note_rnn_hparams,
          note_rnn_type=self.note_rnn_type,
          is_training=False,checkpoint_scope='q_network')
  ...
  def load_validationdata(self,valid_file_name):
    ...
    self.val_lengths=[]
    self.val_weights=[]
    self.q_network.load_dictionary('./ptb_dict.txt')
    max_length=self.dqn_hparams.sequence_len
    with open(valid_file_name,'r') as f:
        for line in f:
          ...
          y=[]
          for i in range(N+1):
            x.append(myrl_tuner.q_network.char_to_int.get(words[i],1))
          ...
          for k in range(1,len(x)):
            ...
            tmpx.extend([myrl_tuner.q_network.char_to_int['<end>']]*(max_length-len(tmpx)))
            tmpy.extend([myrl_tuner.q_network.char_to_int['<end>']]*(max_length-len(tmpy)))   
  ...
  def get_random_note(self):
    ...
    #return np.array([[note_idx]+[3]*(self.note_rnn_hparams.sequence_len-1)])
    return np.array([[note_idx]])
  def reset_composition(self):
    ...
    """
    self.beat = 0
    self.composition = [SYMBOL_START]
    self.composition_direction = 0
    self.leapt_from = None
    self.steps_since_last_leap = 0
    input_batch=[[SYMBOL_START]]
  ...
  def build_graph(self):
    ...
    with tf.name_scope('reward_computation'): 
      self.reward_scores = tf.identity(self.reward_rnn(), name='reward_scores')[:,0,:]
    ...
    with tf.name_scope('taking_action'):
      ...
      # each note).
      self.action_scores = tf.identity(self.q_network(), name='action_scores')[:,0,:]
      self.logits = tf.identity(self.q_network(), name='action_scores')
      tf.summary.histogram(
          'action_scores', self.action_scores)
      ###########################################For perplexity

      self.ground_truth_y=tf.placeholder(tf.int32, [self.dqn_hparams.minibatch_size, self.dqn_hparams.sequence_len])
      self.weights=tf.placeholder(tf.float32, [self.dqn_hparams.minibatch_size, self.dqn_hparams.sequence_len])
      ...
      if self.algorithm == 'g':
        ...
        self.action_softmax = tf.nn.softmax(self.action_scores,
                                            name='action_softmax')
        self.predicted_actions = tf.one_hot(tf.argmax(self.action_scores, dimension=1,name='predicted_actions'),
                                            self.num_actions)
    ...
    with tf.name_scope('estimating_future_rewards'):
      ...
      self.next_action_scores=self.next_action_scores[:,0,:]
    ...
    with tf.name_scope('q_value_prediction'):
      ...
      #self.action_mask = tf.placeholder(tf.float32, (None, 1),name='action_mask')
  ...
  def train(self, num_steps=10000, exploration_period=5000, enable_random=True):
    ...
    Args:
      ...
      enable_random: If False, the model will not be able to act randomly /
        explore.
    """
    with open('record_tl_tuner.txt','w') as f:
      f.write("")
    ...
    for i in range(num_steps):
      ...
      self.composition.append(new_observation[0][0])
      self.beat += 1
      ...
      if i > 0 and i % self.output_every_nth == 0:
        ...
        with open('record_tl_tuner.txt','a') as f:
          f.write('Training iteration'+str(i)+'\n')
          f.write('\tReward for last'+str(self.output_every_nth)+'steps:'+str(r)+'\n')
          f.write('\t\tMusic theory reward:'+str(self.music_theory_reward_last_n)+'\n')
          f.write('\t\tNote RNN reward:'+str(self.note_rnn_reward_last_n)+'\n')
          f.write('\n')

        print('Eval Result', i)
        print('\tReward for last eval', 100, 'steps:', self.eval_avg_reward[-1])
        print('\t\tMusic theory reward:', self.eval_avg_music_theory_reward[-1])
        print('\t\tNote RNN reward:', self.eval_avg_note_rnn_reward[-1])
      ...
      if self.beat % self.num_notes_in_melody == 0 or new_observation[0][0] == SYMBOL_END:
  ...
  def action(self, observation, exploration_period=0, enable_random=True,
    ...
    if enable_random and random.random() < exploration_p:
      ...
      if not sample_next_obs:
        ...
        return np.array([[np.argmax(action)]]), np.array([[np.argmax(action)]]), reward_scores
      else:
        obs_note = rl_tuner_ops.sample_softmax(action_softmax)
        next_obs = np.array([[obs_note]])
  ...
  def training_step(self):
    ...
    if self.num_times_train_called % self.dqn_hparams.train_every_nth == 0:
      ...
      observations = np.empty((len(samples), 1))
      new_observations = np.empty((len(samples), 1))
      ...
      new_observations = np.reshape(new_observations,
                                    (len(samples), -1))
      action_mask=np.reshape(action_mask,(len(samples),-1))
  ...
  def load_tuples(self, filename, dicname):
    dic = {}
    ...
    with open(filename, 'r') as f:
      for line in f:
        ...
        for i in range(len(strs) - 3):
          num_unk = 0
          num_unk += (dic[strs[i]] == SYMBOL_UNK) + (dic[strs[i+1]] == SYMBOL_UNK) + (dic[strs[i+2]] == SYMBOL_UNK)
          if(num_unk <= 1):
            self.tuples[(dic[strs[i]], dic[strs[i+1]], dic[strs[i+2]])] = 1
    ...
    for t in range(num_trials):
      ...
      for _ in range(self.num_notes_in_melody):
        ...
        note_rnn_reward = self.reward_from_reward_rnn_scores(new_observation,
                                                             reward_scores)
        music_theory_reward = self.reward_language_theory(new_observation)
        ...
        last_observation = new_observation
        #jing 
        if(last_observation == SYMBOL_END):
          break;
  ...
  def get_perplexity(self):
      batch_size=self.dqn_hparams.minibatch_size
      N=len(self.val_dataX)
      L=int(N/self.dqn_hparams.minibatch_size-1)
      ...
      for i in range(L):
        ...
        initial_state=np.zeros((batch_size, self.q_network.cell.state_size))

        batch_loss, softmax, self.q_network.state_value = self.session.run(
                [self.batch_loss,self.action_softmax, self.q_network.state_tensor],
                {self.q_network.melody_sequence: x,
                 self.q_network.initial_state: initial_state,
                 self.ground_truth_y: y,
                 self.weights: w,
                 self.q_network.lengths: length})
  ...
  def collect_reward(self, obs, action, reward_scores):
    ...
    r1=self.reward_penalize_repeating(action)
    r2=self.reward_language_theory(action)
    self.music_theory_reward_last_n += (r1+r2) * self.reward_scaler
    #r3=self.reward_penalize_autocorrelation(action)
    r3 = 0.0
    #reward += self.reward_penalize_autocorrelation(action)


    reward=(r1+r2+r3) * self.reward_scaler+note_rnn_reward

    '''
    print ('note_rnn_reward',note_rnn_reward)
    print("reward_penalize_repeating",r1)
    print("reward_language_theory",r2)
    print("reward_penalize_autocorrelation",r3)
    print ("collect reward=",reward)
    print ("\n")
    '''
    return reward
  ...
  def reward_from_reward_rnn_scores(self, action, reward_scores):
    ...
    action_note = action[0][0]
    normalization_constant = logsumexp(reward_scores)
    #return reward_scores[action_note] - normalization_constant
    if action_note == SYMBOL_UNK:
      return 0.0
    else:
      return np.exp(reward_scores[action_note])/np.sum(np.exp(reward_scores)) * 100
  ...
  def get_reward_rnn_scores(self, observation, state):
    ...
    rewards, = self.session.run(
        ...
        {self.reward_rnn.melody_sequence: input_batch,
         ...
         self.reward_rnn.lengths: lengths})
    return rewards
  def reward_language_theory(self, action):
    action = action[0][0]
    
    reward = 0.0
    number_unk = 0
    for i in range(len(self.composition)-1, -1, -1):
      if(self.composition[i] == SYMBOL_UNK):
        number_unk += 1
    if (number_unk > 4):
      reward += -100.0
    ...
    num_exist = 0.0
    list_tuples = []
    for i in range(len(self.composition) - 3):
      num_tri += 1
      tp = (self.composition[i], self.composition[i+1], self.composition[i+2])
      if(self.tuples.has_key(tp) and (not tp in list_tuples)):
        num_exist += 1
        list_tuples.append(tp)
    if(len(self.composition) == 2):
      num_tri += 1
      if(self.tuples.has_key((self.composition[-2], self.composition[-1], action))):
        num_exist += 1
        list_tuples.append((self.composition[-2], self.composition[-1], action))
    if(num_tri > 0):
  ...
  def detect_repeating_notes(self, action_note):
    ...
    if(len(self.composition)<1):
     return False
    return self.composition[-1]==action_note
  ...
  def reward_penalize_autocorrelation(self,
    ...
    Returns:
      Float reward value.
    """
    composition = self.composition + [action[0][0]]
    lags = [1, 2, 3]
    sum_penalty = 0
    for lag in lags:
  ...
  def reward_high_low_unique(self, action, reward_amount=3.0):
    ...
    composition = np.append(composition, action[0][0])
  ...
  def generate_music_sequence(self, title='rltuner_sample',
                              ...
                              length=None, most_probable=False,num_of_sentence=50,saved_file='generated_sequence.txt'):
    ...
    while (num_of_generated<num_of_sentence):
        num_of_generated+=1
        next_obs = self.prime_internal_models()
        self.reset_composition()
        
        tf.logging.info('Priming with note %s', next_obs[0][0])
        print('Priming with note %s'%next_obs[0][0])
        lengths = np.full(self.q_network.batch_size, 1, dtype=int)
        
        if visualize_probs:
          prob_image = np.zeros((self.input_size, length))
        generated_seq=next_obs
        generated_str = ''+self.q_network.int_to_char[next_obs[0][0]]
        ...
        for i in range(length):
          ...
          generated_str = generated_str+' '+self.q_network.int_to_char[sample]
          next_obs = np.array([[sample]])
          self.composition.append(sample)
          if(self.q_network.int_to_char[sample]=='<end>' or len(self.composition)> self.num_notes_in_melody):
              break
                                                       
        tf.logging.info('Generated sequence: %s', generated_str)
        # TODO(natashamjaques): Remove print statement once tf.logging outputs
        # to Jupyter notebooks (once the following issue is resolved:
        # https://github.com/tensorflow/tensorflow/issues/3047)
  ...
  def restore_from_directory(self, directory=None, checkpoint_name=None,
    ...
    Args:
      ...
      reward_file_name: The name of the .npz file where the stored
        rewards are saved. If None, wilsessl not attempt to load stored
        rewards.
    """
    if directory is None:
      directory = self.output_dir
...
if __name__ =='__main__':
  ...
  myrl_tuner=RLTuner(note_rnn_checkpoint_dir='./ckp2',output_dir='./text_generated',output_every_nth=10000)

  myrl_tuner.load_validationdata("./ptb.valid.txt")
  
  myrl_tuner.load_tuples('ptb.train.txt', 'ptb_dict.txt')
  #myrl_tuner.train(num_steps=100000)
  #myrl_tuner.restore_from_directory(directory='./model_dir')
  #myrl_tuner.restore_from_directory(directory='./model_correlated_no_repeat_language')
  myrl_tuner.restore_from_directory(directory='./text_generated')
  myrl_tuner.generate_music_sequence(most_probable=False,length=20,saved_file='new_model.txt',num_of_sentence=200)
  pdb.set_trace()
